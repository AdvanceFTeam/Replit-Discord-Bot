...
module.exports = {
  name: 'ahegao',
  description: '**(NSFW)** Returns a randomly selected ahegao content.',
  usage: '**!ahegao**',
  execute(message) {
    ...
    // Make a GET request to the API to fetch NSFW content of type "ahegao"
    axios.get(`${API_BASE_URL}/img/nsfw?type=ahegao`)
      .then(response => {
        const nsfwContent = response.data;

        // Create a new MessageEmbed to display the NSFW content
        const embed = new MessageEmbed()
          .setTitle('NSFW Content - Ahegao')
